module PopularPageTags
  ...
  desc %{
    ...
    You may limit the results to a certain author by providing an @author@
    attribute. The value should match the author's @login@.
    
    *Usage:*
    <pre><code><r:popular:pages:each [limit='3' author='amy']>...</r:popular:pages:each></code><pre>
  }
  tag 'popular:pages:each' do |tag|
    ...
    results = []
    tag.locals.popular_pages = Page.find_popular(limit, author)
